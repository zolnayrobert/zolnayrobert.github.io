...
const products = products4Inventory.map(product => new Product(product.id, product.name, product.price, product.description));
inventory.uploadProducts(products);
console.log(inventory.listAllProducts());

console.log('-----ADD NEW USER------');
const user = new User(1,'Zolnay Róbert', 'zolnayrobert@gmail.com');
...
const order = new Order(user, inventory);
console.log(order.orderStatus);

console.log('-----ADD 2 CART------');
if(product) order.add2cart(product);
console.log(order.orderStatus);
console.log(inventory.listAllProducts());
